...
int main(void)
{
 ...
 // freeRtos stuff end
      ...
      rightChopstickQueue =  xQueueCreate(3, sizeof(rightChopStickTicket));

      xQueueSend(rightChopstickQueue,&rightChopStickTicket,portMAX_DELAY);
      xQueueSend(rightChopstickQueue,&rightChopStickTicket,portMAX_DELAY);
      xQueueSend(rightChopstickQueue,&rightChopStickTicket,portMAX_DELAY);
...
static void philosopher_1( void *pvParameters )
{
	...
	for( EVER )
	{
		xQueueReceive(rightChopstickQueue,&rightChopStickTicket,portMAX_DELAY);
